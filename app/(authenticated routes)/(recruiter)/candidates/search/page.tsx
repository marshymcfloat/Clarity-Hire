...
export default function CandidateSearchPage() {
  ...
  const { data: jobs } = useQuery({
    ...
    queryFn: async () => {
      try {
        const res = await getCompanyJobsAction();
        return res || [];
      } catch (e) {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      const payload = {
        ...
        jobId: filters.jobId || undefined, // Pass job ID if selected
        filters: {
  ...
  const handleViewReport = (candidateId: string) => {
    ...
    router.push(
      `/candidates/${candidateId}/match-report?jobId=${filters.jobId}`,
    );
  };
          ...
          {isLoading ? (
            ...
            <div className="space-y-4">
              <p className="text-sm text-muted-foreground mb-2">
                Found {data?.results?.length || 0} candidates
              ...
              {data?.results?.map((candidate: any) => (
                <CandidateResultCard
                  ...
                  candidate={{
                    ...candidate,
                    matchScore: (candidate.matchScore || 0) * 100,
                    skills: candidate.metadata?.skills
                      ? candidate.metadata.skills.split(",")
                      : [],
                    location: candidate.metadata?.location || "Unknown",
                    name: candidate.metadata?.name,
                    email: candidate.metadata?.email,
                    matchExplanation: filters.jobId
                      ? "Match score based on semantic similarity to job description."
                      : "Matched based on query similarity.",
                  }}
